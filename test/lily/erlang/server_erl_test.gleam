...
@target(erlang)
pub fn server_max_topics_rejects_past_limit_test() {
...
@target(erlang)
pub fn server_unregister_topic_frees_slot_test() {
  let srv = server_with_max_topics(1)
  ...
  // Fixed topic fills the only slot, so the parametric create is rejected.
  server.incoming(srv, client_id: "c1", bytes: encode_subscribe("room:1"))
  case recv(subj) {
    Ok(bytes) ->
      transport.decode(bytes, serialiser: ser())
  ...
  // Unregister the fixed topic, freeing the slot, then the create succeeds.
  topic.stop(t)
  process.sleep(20)
  server.incoming(srv, client_id: "c1", bytes: encode_subscribe("room:2"))
  process.sleep(20)
  case recv(subj) {
    ...
    Ok(bytes) ->
      case transport.decode(bytes, serialiser: ser()) {
        Ok(transport.Rejected(_, _)) -> should.fail()
